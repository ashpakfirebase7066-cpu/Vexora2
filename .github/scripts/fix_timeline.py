...
# Add callback parameter; default keeps any older call sites compiling.
s = s.replace(
    'onResize: ((Float) -> Unit)?,\n    toolbarVisible: Boolean,',
    'onResize: ((Float) -> Unit)?,\n    onResizeEnd: (() -> Unit)? = null,\n    toolbarVisible: Boolean,'
)

# Ensure every TimelineClip call receives the callback.
s = s.replace(
...
s = s.replace('tint = if (arrowActive) Color.White else Color.Black,', 'tint = Color.Black,')
